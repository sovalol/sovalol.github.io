...
time_value.addEventListener('change', () => {
	...
	if (time < 0) {
			isopen_value.textContent = 'Такого не может быть';
			mainList.open = false;
			...
			isopen_value.textContent = 'Чувак, в сутках всего 24 часа!';
			mainList.open = false;
		};
...
budget_btn.addEventListener('click', () => {
	...
	count_budget_value.value = budj / 30;
...
discount_value.addEventListener('click', () => {
	...
	price = prompt('Введите стоимость товара', '');
		if (mainList.discount == true) {
			discount_value.textContent = ('Стоимость товара со скидкой ' + price * 0.8 + ' руб');
			discount_value.style.backgroundColor = 'green';

		} else if (mainList.discount == false) {
			discount_value.textContent = 'Мы вам скидку не дадим!';
		}
